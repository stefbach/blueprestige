  ...
  <section id="visite" style="padding:130px 40px;background:#101A4D;color:#fff">
    <div style="display:flex;flex-wrap:wrap;align-items:flex-end;justify-content:space-between;gap:30px;padding-bottom:50px">
      ...
      <div style="display:flex;flex-wrap:wrap;gap:2px;background:rgba(255,255,255,.16)">
        ${each(ongletsBiens, (t) => `
          <button type="button" data-action="onglet" data-id="${esc(t.id)}" style="padding:14px 22px;border:0;background:${t.bg};color:${t.fg};font-size:11px;letter-spacing:.18em;text-transform:uppercase;white-space:nowrap;cursor:pointer">${esc(t.label)}</button>
        `)}
      </div>
    </div>
    <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(360px,1fr));gap:44px;align-items:stretch">
      <div style="display:flex;flex-direction:column;gap:22px">
        ...
        <p style="margin:0;font-size:12px;line-height:1.7;color:#8E99DE;font-weight:300">Cliquez une pièce du plan pour la parcourir.</p>
      </div>
      <div style="display:flex;flex-direction:column;gap:0;background:rgba(255,255,255,.05)">
        ...
        <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:34px;padding:36px 34px">
          <div style="display:flex;flex-direction:column;gap:14px">
            <h3 style="margin:0;font-family:'Cormorant Garamond',serif;font-weight:400;font-size:30px;line-height:1.1;color:#fff">${esc(pieceActive.label)}</h3>
            <p style="margin:0;font-size:15px;line-height:1.8;color:#B9C1EC;font-weight:300;text-wrap:pretty">${esc(pieceActive.texte)}</p>
          </div>
          <div style="display:flex;flex-direction:column;gap:20px;padding-left:30px;border-left:1px solid rgba(255,255,255,.18)">
            <span style="display:flex;flex-direction:column;gap:6px">
              <span style="font-size:9px;letter-spacing:.24em;text-transform:uppercase;color:#8E99DE">Lumière</span>
              <span style="font-family:'Cormorant Garamond',serif;font-style:italic;font-size:18px;color:#fff">${esc(pieceActive.lumiere)}</span>
            </span>
            ${when(artPairing, () => `
              <span style="display:flex;flex-direction:column;gap:6px">
                <span style="font-size:9px;letter-spacing:.24em;text-transform:uppercase;color:#8E99DE">Œuvre en place</span>
  ...
  <section id="art" style="padding:130px 40px">
    <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(320px,1fr));gap:70px;padding-bottom:56px">
      <div style="display:flex;flex-direction:column;gap:20px">
        <span style="font-size:11px;letter-spacing:.34em;text-transform:uppercase;color:#2A3BC4">L'atelier</span>
        <h2 style="margin:0;font-family:'Cormorant Garamond',serif;font-weight:300;font-size:clamp(34px,3.4vw,54px);line-height:1.08">Les œuvres de Juliana</h2>
      </div>
      <p style="margin:0;align-self:end;max-width:56ch;font-size:16px;line-height:1.8;color:#3C4470;font-weight:300;text-wrap:pretty">Peinture au couteau, touches denses, bleus profonds et rouges intenses. Ses toiles sont chez des collectionneurs dans le monde entier, notamment via <a href="https://www.artsper.com" target="_blank" rel="noopener">Artsper</a> — et parfois sur les murs des biens qu'elle présente.</p>
    </div>
    <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(270px,1fr));gap:28px">
      ${each(toiles, (t) => `
        <figure style="display:flex;flex-direction:column;gap:16px;margin:0;cursor:pointer" data-action="toile" data-id="${esc(t.id)}">
          <div style="position:relative;aspect-ratio:1;background:#F4F6FE">
            ${imageSlot({ src: t.img, placeholder: t.slot, fit: 'cover' })}
            <span style="position:absolute;top:0;right:0;padding:9px 14px;background:#fff;font-size:9px;letter-spacing:.2em;text-transform:uppercase;white-space:nowrap;color:#2A3BC4;pointer-events:none">${esc(t.badge)}</span>
          </div>
          <figcaption style="display:flex;flex-direction:column;gap:6px">
            <span style="font-family:'Cormorant Garamond',serif;font-size:22px;line-height:1.15">${esc(t.titre)}</span>
            <span style="font-size:11px;letter-spacing:.12em;color:#6B739E">${esc(t.meta)}</span>
          </figcaption>
        </figure>
      `)}
    </div>
  ...
  <div data-action="close-lightbox" style="position:fixed;inset:0;z-index:90;display:grid;place-items:center;padding:60px;background:rgba(11,18,54,.94);animation:bpFade .3s ease both;cursor:zoom-out">
    <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(260px,1fr));gap:44px;align-items:center;max-width:1100px;width:100%">
      <div style="position:relative;aspect-ratio:1;background:#0B1236">
        ${imageSlot({ src: lightbox.img, placeholder: lightbox.titre, fit: 'contain' })}
      </div>
      <div style="display:flex;flex-direction:column;gap:16px;color:#fff">
        <span style="font-size:10px;letter-spacing:.26em;text-transform:uppercase;color:#9AA6E8">${esc(lightbox.badge)}</span>
        <h3 style="margin:0;font-family:'Cormorant Garamond',serif;font-weight:300;font-size:42px;line-height:1.05">${esc(lightbox.titre)}</h3>
        <p style="margin:0;font-size:14px;letter-spacing:.1em;color:#B9C1EC">${esc(lightbox.meta)}</p>
        <p style="margin:0;font-size:15px;line-height:1.8;color:#B9C1EC;font-weight:300;text-wrap:pretty">${esc(lightbox.note)}</p>
      </div>
    </div>
  </div>
  `

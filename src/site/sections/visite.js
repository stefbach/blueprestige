...
export function renderVisite(ctx) {
  ...
  return `
  <section id="visite" style="padding:clamp(64px,11vw,130px) clamp(20px,5vw,40px);background:#101A4D;color:#fff">
    ...
    <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(min(360px,100%),1fr));gap:44px;align-items:stretch">
      <div style="display:flex;flex-direction:column;gap:22px">
        ...
        <div data-plan-pieces style="display:grid;grid-template-columns:repeat(6,1fr);grid-template-rows:repeat(4,1fr);gap:5px;height:clamp(320px,72vw,440px);padding:14px;border:1px solid rgba(255,255,255,.20)">
          ${each(pieces, (r, i) => `
      ...
      <div style="display:flex;flex-direction:column;gap:0;background:rgba(255,255,255,.05)">
        ...
        <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(min(220px,100%),1fr));gap:34px;padding:36px 34px">
          ...
          <div data-piece-meta style="display:flex;flex-direction:column;gap:20px;padding-left:30px;border-left:1px solid rgba(255,255,255,.18)">
...
export function renderArt(ctx) {
  const { toiles } = ctx
  return `
  <section id="art" style="padding:clamp(64px,11vw,130px) clamp(20px,5vw,40px)">
    <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(min(320px,100%),1fr));gap:clamp(36px,7vw,70px);padding-bottom:56px">
    ...
    <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(min(270px,100%),1fr));gap:28px">
      ${each(toiles, (t2) => `
...
export function renderLightbox(ctx) {
  ...
  if (!lightbox) return ''
  return `
  <div data-action="close-lightbox" style="position:fixed;inset:0;z-index:90;display:grid;place-items:center;padding:clamp(16px,5vw,60px);overflow:auto;background:rgba(11,18,54,.94);animation:bpFade .3s ease both;cursor:zoom-out">
    <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(min(260px,100%),1fr));gap:44px;align-items:center;max-width:1100px;width:100%">
